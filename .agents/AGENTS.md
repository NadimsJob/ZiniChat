...
* **Authentication**: The MCP server connects to the VPS (`76.13.187.85`) using the local machine's SSH Private Key. The path is configured in `backend/.env` via the `VPS_PRIVATE_KEY_PATH` variable (e.g. `C:/Users/Name/.ssh/id_rsa`). The VPS targets are `/var/www/zinichat-test` and `/var/www/zinichat-live`.
