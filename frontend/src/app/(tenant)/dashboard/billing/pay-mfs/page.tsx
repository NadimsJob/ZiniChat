...
function PayMfsContent() {
 ...
 className="text-emerald-600 font-bold hover:underline flex items-center gap-1 transition-colors bg-emerald-50 px-3 py-1.5 rounded-lg shrink-0"
 >
 ...
 <span>টাকার পরিমাণ: <strong className="font-mono text-emerald-600 text-[14px]">{formatBdtDirect(qrPayload?.amount || payment?.amountBdt || 0)}</strong></span>
 ...
 <span className="font-bold text-emerald-600 shrink-0">2.</span>
 <span>
 "<strong className="text-emerald-600 ">{selectedAccount.accountType === 'MERCHANT' ? 'Payment / Merchant Pay' : (selectedAccount.accountType === 'AGENT' ? 'Cash Out' : 'Send Money')}</strong>" অপশনে ক্লিক করুন।
 </span>
 </div>
 <div className="flex gap-2 items-center">
 <span className="font-bold text-emerald-600 shrink-0">3.</span>
