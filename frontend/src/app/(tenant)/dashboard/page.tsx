...
export default function TenantDashboardOverview() {
 ...
 <div className="bg-white rounded-2xl border border-amber-500/20 shadow-sm overflow-hidden relative">
 ...
 <p className="text-xl font-bold text-slate-900 ">৳{stats?.orders?.revenue?.toLocaleString() || 0}</p>
 </div>
 <div>
 <p className="text-xs text-slate-500 mb-1">{language === 'en' ? 'Total Products' : 'মোট প্রোডাক্ট'}</p>
 <p className="text-xl font-bold text-slate-900 ">{stats?.orders?.totalProducts || 0}</p>
 </div>
 ...
 <p className="text-[11px] text-amber-600 font-bold uppercase mb-1">Pending Orders</p>
 <p className="text-2xl font-extrabold text-amber-700 ">{stats?.orders?.pending || 0}</p>
 </div>
 <div className="flex-1 bg-emerald-50 p-3 rounded-xl border border-emerald-100 ">
 <p className="text-[11px] text-emerald-600 font-bold uppercase mb-1">Completed</p>
 <p className="text-2xl font-extrabold text-emerald-700 ">{stats?.orders?.completed || 0}</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="bg-white rounded-2xl border border-secondary/20 shadow-sm overflow-hidden relative">
 ...
 <p className="text-3xl font-extrabold text-slate-900 ">{stats?.leads?.total || 0}</p>
 </div>
 <div className="bg-primary/5 p-4 rounded-xl border border-primary/10">
 <p className="text-xs text-primary font-bold uppercase mb-1">{language === 'en' ? 'New This Month' : 'নতুন লিড'}</p>
 <div className="flex items-center gap-2">
 <p className="text-3xl font-extrabold text-slate-900 ">{stats?.leads?.newThisMonth || 0}</p>
 ...
 <h3 className="font-bold text-slate-900 text-[13px] mb-4 flex items-center gap-2">
 <Activity className="w-4 h-4 text-primary" />
 {language === 'en' ? 'Quick Actions' : 'কুইক একশন'}
 </h3>
 <div className="flex flex-col gap-2">
 <Link href="/dashboard/settings/whatsapp" className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 hover:bg-slate-100 :bg-zinc-800 transition-colors group">
 <div className="flex items-center gap-3">
 <div className="w-7 h-7 rounded bg-green-500/10 text-green-600 flex items-center justify-center">
 <PhoneCall className="w-3.5 h-3.5" />
 </div>
 <span className="text-sm font-medium text-slate-700 ">Add WhatsApp</span>
 </div>
 <ArrowRight className="w-3.5 h-3.5 text-slate-400 group-hover:translate-x-1 transition-transform" />
 </Link>

 <Link href="/dashboard/inbox" className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 hover:bg-slate-100 :bg-zinc-800 transition-colors group">
 <div className="flex items-center gap-3">
 ...
 {stats?.features?.includes('ecommerce') && (
 <Link href="/dashboard/products" className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 hover:bg-slate-100 :bg-zinc-800 transition-colors group">
 <div className="flex items-center gap-3">
 ...
 <Link href="/dashboard/broadcast" className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 hover:bg-slate-100 :bg-zinc-800 transition-colors group">
 <div className="flex items-center gap-3">
 <div className="w-7 h-7 rounded bg-violet-500/10 text-violet-600 flex items-center justify-center">
 <Megaphone className="w-3.5 h-3.5" />
 </div>
 <span className="text-sm font-medium text-slate-700 ">New Broadcast</span>
 </div>
 <ArrowRight className="w-3.5 h-3.5 text-slate-400 group-hover:translate-x-1 transition-transform" />
 </Link>
 )}
