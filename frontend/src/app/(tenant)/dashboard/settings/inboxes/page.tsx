        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-3.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <h2 className="text-xs font-bold text-slate-700">
              {connections.length} {connections.length === 1 ? 'active inbox' : 'active inboxes'}
            </h2>
          ...
          <div className="divide-y divide-slate-100">
            {loading ? (
              ...
              connections.map(conn => {
                const isActive = conn.status === 'active' || conn.status === 'connected' || conn.isConnected === true || !conn.status;
                return (
                  <div key={conn.id} className="p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-slate-50/70 transition-colors group">
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-3 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100 opacity-100 transition-opacity">
