...
export default function WidgetSettings({ connections }: { connections: any[] }) {
 ...
 if (hasFeature === false) {
 return (
 <div className="p-8 text-center bg-zinc-50 rounded-xl border border-zinc-200 flex flex-col items-center justify-center animate-in fade-in duration-300">
 ...
 <div key={plan.id} className="p-4 bg-white border border-slate-200 rounded-xl flex flex-col justify-between items-center text-center shadow-sm hover:shadow-md transition-shadow">
 <h4 className="font-bold text-slate-900 text-[15px]">{plan.name}</h4>
 ...
 return (
 <div className="p-6 space-y-6 animate-in fade-in zoom-in-95 duration-300 bg-white ">
 ...
 {!activeConnection ? (
 <div className="bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-xl text-[13px] font-medium">
 {language === 'en' 
 ...
 <pre className="bg-slate-900 text-green-400 p-4 rounded-lg text-sm overflow-x-auto border border-slate-800 font-mono">
 {scriptCode}
 </pre>
 <button 
 onClick={handleCopy}
