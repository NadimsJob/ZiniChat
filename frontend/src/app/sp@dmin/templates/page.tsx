...
export default function SuperadminTemplatesPage() {
  ...
  const [monPageSize, setMonPageSize] = useState(10);

  // Library state
  const [libTemplates, setLibTemplates] = useState<any[]>([]);
  const [libLoading, setLibLoading] = useState(false);
  const [libPage, setLibPage] = useState(1);
  const [libPageSize, setLibPageSize] = useState(10);
      ...
      {activeTab === 'monitoring' && (
        <>
          <div className="flex justify-end">
            <button onClick={fetchMonitoring} className="flex items-center gap-1.5 px-3 py-1.5 bg-surface-hover text-zinc-200 text-[12px] font-bold rounded-xl hover:bg-surface-hover/80 transition-all cursor-pointer">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-between items-center bg-white dark:bg-surface border border-slate-200 dark:border-surface-hover p-3 rounded-2xl shadow-sm">
            <div className="relative w-full sm:w-72">
              ...
              <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                ...
                className="w-full bg-slate-50 dark:bg-background border border-slate-200 dark:border-surface-hover text-slate-900 dark:text-white rounded-xl pl-9 pr-3 py-1.5 text-[12px] outline-none focus:border-primary" />
            </div>
            <div className="flex gap-2 w-full sm:w-auto">
              {['ALL', 'PENDING', 'APPROVED', 'REJECTED'].map(st => (
                <button key={st} onClick={() => setFilterStatus(st)}
                  className={`px-3 py-1.5 rounded-lg text-[12px] font-bold transition-all cursor-pointer ${filterStatus === st ? 'bg-primary text-primary-foreground' : 'bg-slate-100 dark:bg-surface-hover/30 text-slate-700 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-200'}`}>
                  {st}
                </button>
              ))}
            </div>
          ...
          <div className="bg-white dark:bg-surface border border-slate-200 dark:border-surface-hover rounded-2xl overflow-hidden shadow-sm dark:shadow-xl p-4 space-y-4">
            {monLoading ? (
              <AdminLoader message="Loading Meta WhatsApp templates status..." />
            ) : (
              <>
                <div className="space-y-3">
                  {filteredMon.length === 0 ? (
                    <div className="text-center py-12 text-slate-500 dark:text-zinc-500 text-[13px]">{language === 'en' ? 'No templates found.' : 'কোনো টেমপ্লেট পাওয়া যায়নি।'}</div>
                  ) : filteredMon.slice((monPage - 1) * monPageSize, monPage * monPageSize).map(template => (
                    <div key={template.id} className="p-4 border border-slate-200 dark:border-surface-hover rounded-xl bg-slate-50 dark:bg-background/40 hover:border-primary/30 transition-colors">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            <h3 className="font-bold text-[14px] text-slate-900 dark:text-white">{template.name}</h3>
                            <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold ${template.status === 'APPROVED' ? 'bg-green-500/10 text-green-600 dark:text-green-500' : template.status === 'REJECTED' ? 'bg-red-500/10 text-red-600 dark:text-red-500' : 'bg-orange-500/10 text-orange-600 dark:text-orange-500'}`}>{template.status}</span>
                            {template.status === 'APPROVED' && (
                              <button onClick={() => setShowPromote(template)} className="flex items-center gap-1 px-2 py-0.5 bg-primary/10 text-primary text-[10px] font-bold rounded-full hover:bg-primary/20 transition-all border border-primary/20 cursor-pointer">
                                <ArrowUpCircle className="w-3 h-3" /> Promote to Library
                              </button>
                            )}
                          </div>
                          <p className="text-[12px] text-slate-600 dark:text-zinc-400 mt-0.5">
                            Tenant: <strong className="text-slate-900 dark:text-zinc-200">{template.tenant?.businessName || 'Unknown'}</strong> &bull; {template.category} &bull; {template.language || 'bn'}
                          </p>
                        </div>
                        {template.metaTemplateId && <span className="text-[11px] font-mono text-slate-600 dark:text-zinc-400 bg-slate-200 dark:bg-surface-hover px-2 py-0.5 rounded">ID: {template.metaTemplateId}</span>}
                      </div>
                      {template.bodyText && <div className="bg-white dark:bg-background/80 p-3 rounded-lg border border-slate-200 dark:border-surface-hover text-[12px] whitespace-pre-wrap font-mono text-slate-900 dark:text-zinc-300 mb-2">{template.bodyText}</div>}
                      {template.rejectionReason && <div className="p-2 bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 text-[12px] rounded-lg"><strong>Meta Rejection Reason:</strong> {template.rejectionReason}</div>}
                    </div>
                  ))}
                </div>
                ...
                <AdminPagination
                  currentPage={monPage}
                  totalItems={filteredMon.length}
                  pageSize={monPageSize}
                  onPageChange={setMonPage}
                  onPageSizeChange={setMonPageSize}
                />
              </>
            )}
          </div>
      ...
      {activeTab === 'library' && (
        <>
          <div className="flex justify-between items-center">
            <p className="text-[13px] text-slate-600 dark:text-zinc-400 font-medium">{libTemplates.length} {language === 'en' ? 'templates in library' : 'টি টেমপ্লেট লাইব্রেরিতে আছে'}</p>
            <button onClick={() => { setEditTarget(null); setShowModal('create'); }}
              className="flex items-center gap-1.5 px-4 py-2 bg-primary text-white text-[12px] font-bold rounded-xl hover:bg-primary/90 transition-all shadow-lg shadow-primary/20 cursor-pointer">
          ...
          {suggestedTemplates.length > 0 && (
            <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-4">
              <h3 className="text-[13px] font-bold text-amber-500 dark:text-amber-400 mb-3 flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {suggestedTemplates.slice(0, 6).map((t: any) => (
                  <div key={t.id} className="bg-white dark:bg-background/60 border border-amber-500/20 rounded-xl p-3 flex justify-between items-start gap-2 shadow-sm">
                    <div className="min-w-0">
                      <p className="text-[12px] font-bold text-slate-900 dark:text-zinc-200 truncate">{t.name}</p>
                      <p className="text-[11px] text-slate-500 dark:text-zinc-400 truncate">{t.tenant?.businessName} · {t.category}</p>
                    </div>
                    <button onClick={() => setShowPromote(t)} className="shrink-0 flex items-center gap-1 px-2 py-1 bg-amber-500/10 text-amber-600 dark:text-amber-400 text-[10px] font-bold rounded-lg hover:bg-amber-500/20 transition-all border border-amber-500/20 cursor-pointer">
          ...
          <div className="bg-white dark:bg-surface border border-slate-200 dark:border-surface-hover rounded-2xl overflow-hidden shadow-sm dark:shadow-xl">
            {libLoading ? (
              <AdminLoader message="Loading global prompt & message templates library..." />
              ...
              <>
                <div className="overflow-x-auto">
                  <table className="w-full text-[12px]">
                    <thead className="border-b border-slate-200 dark:border-surface-hover bg-slate-100/90 dark:bg-background/40 text-slate-700 dark:text-zinc-400">
                      <tr className="text-[11px]">
                        <th className="px-4 py-3 text-left font-medium">Title</th>
                        <th className="px-4 py-3 text-left font-medium">Tag / Category</th>
                        <th className="px-4 py-3 text-left font-medium">Language</th>
                        <th className="px-4 py-3 text-center font-medium">Used By</th>
                        <th className="px-4 py-3 text-center font-medium">Visibility</th>
                        <th className="px-4 py-3 text-right font-medium">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-surface-hover text-slate-900 dark:text-zinc-200">
                      {libTemplates.slice((libPage - 1) * libPageSize, libPage * libPageSize).map(tpl => (
                        <tr key={tpl.id} className="hover:bg-slate-50 dark:hover:bg-background/30 transition-colors">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              {tpl.isFeatured && <Star className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400 shrink-0" />}
                              <span className="font-bold text-slate-900 dark:text-zinc-200 truncate max-w-[180px]">{tpl.title}</span>
                            </div>
                            <p className="text-[11px] text-slate-500 dark:text-zinc-400 truncate max-w-[200px] mt-0.5">{tpl.bodyText?.slice(0, 50)}...</p>
                          </td>
                          <td className="px-4 py-3">
                            <span className="bg-slate-100 dark:bg-surface-hover border border-slate-200 dark:border-surface-hover px-2 py-0.5 rounded text-[11px] text-slate-800 dark:text-zinc-300 block w-fit mb-1 font-medium">{tpl.categoryTag}</span>
                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${tpl.category === 'MARKETING' ? 'bg-purple-500/10 text-purple-600 dark:text-purple-400' : tpl.category === 'UTILITY' ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400' : 'bg-gray-500/10 text-gray-600 dark:text-gray-400'}`}>{tpl.category}</span>
                          </td>
                          <td className="px-4 py-3 text-slate-600 dark:text-zinc-400 font-medium">{tpl.language}</td>
                          <td className="px-4 py-3 text-center">
                            <span className="font-bold text-primary text-[13px]">{tpl.usageCount}</span>
                            <p className="text-[10px] text-slate-500 dark:text-zinc-500">tenants</p>
                          </td>
                      <td className="px-4 py-3 text-center">
            ...
            <AdminPagination
              currentPage={libPage}
              totalItems={libTemplates.length}
              pageSize={libPageSize}
              onPageChange={setLibPage}
              onPageSizeChange={setLibPageSize}
            />
          </>
        )}
      </div>
