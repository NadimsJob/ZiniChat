...
import { Settings2, X, Eye, LogIn, Loader2, CheckCircle, Trash2 } from 'lucide-react';
...
export default function TenantsPage() {
  ...
  const handleImpersonate = async (tenantId: string, tenantName: string) => {
    try {
      ...
      if (data.access_token) {
        ...
        window.open(`/sp@dmin/impersonate?token=${data.access_token}&role=${userRole}&tenant=${encTenant}`, '_blank');
      } else {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-2 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h1 className="text-[15px] font-bold tracking-tight text-slate-900 dark:text-white">Tenants</h1>
        <p className="text-slate-600 dark:text-zinc-400 mt-2">Manage all registered businesses and their platform access.</p>
      </div>

      <div className="bg-white dark:bg-surface border border-slate-200/80 dark:border-surface-hover rounded-xl overflow-hidden shadow-sm dark:shadow-lg">
        {loading ? (
          <AdminLoader message="Loading registered tenants & workspaces..." />
        ) : (
          <>
                  ...
                  {tenants.length === 0 ? (
                    ...
                    tenants.slice((page - 1) * pageSize, page * pageSize).map(tenant => (
                      <tr key={tenant.id} className="hover:bg-slate-50 dark:hover:bg-surface-hover/30 transition-colors">
                          ...
                          <div className="flex flex-col gap-0.5">
                            <div className="flex items-center gap-1.5">
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${
                                tenant.subscriptionStatus === 'active' ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20' : 
                                tenant.subscriptionStatus === 'past_due' ? 'bg-orange-500/10 text-orange-700 dark:text-orange-400 border-orange-500/20' : 
                                'bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-500 border-slate-200 dark:border-zinc-700'
                              }`}>
                              ...
                              {(tenant.customFeatures !== null || tenant.customPriceUsd !== null || tenant.customMessageQuota !== null || tenant.customPlanName !== null || tenant.customSeatLimit !== null) && (
                                <span className="px-1.5 py-0.5 rounded text-[9px] font-semibold bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-500/20">
                                  Customized
                                </span>
                              )}
                            </div>
                            <span className="text-[11px] text-slate-600 dark:text-zinc-400 font-medium truncate max-w-[120px]">
                        ...
                        <td className="px-3 py-2 text-slate-800 dark:text-zinc-300">
                          {tenant.aiQuota ? (
                            <span className="text-xs">
                              <strong className="text-slate-900 dark:text-zinc-100">{tenant.aiQuota.used}</strong> / {tenant.aiQuota.limit}
                            </span>
                          ) : 'N/A'}
                        </td>
                        <td className="px-3 py-2">
                          <select 
                            ...
                            className="bg-slate-50 dark:bg-[#09090b] border border-slate-300 dark:border-zinc-800 rounded px-2 py-1 text-[11px] text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500 max-w-[130px] truncate cursor-pointer"
                          >
                            {aiConfigs.map(c => (
                        ...
                        <td className="px-3 py-2">
                          <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                            tenant.status === 'active' ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400' : 'bg-red-500/10 text-red-700 dark:text-red-400'
                          }`}>
                        ...
                        <td className="px-3 py-2 text-right">
                          <div className="flex flex-wrap justify-end items-center gap-1.5 sm:gap-2">
                            ...
                            <button 
                              ...
                              className="text-xs px-2.5 py-1.5 rounded-lg font-medium bg-slate-100 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 hover:bg-slate-200 dark:hover:bg-zinc-700 transition-colors flex items-center gap-1.5 cursor-pointer"
                            >
      ...
      {editingTenant && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white dark:bg-[#121214] border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-2xl max-w-lg w-full p-6 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto text-slate-900 dark:text-white">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">Customize Plan</h3>
              <button onClick={() => setEditingTenant(null)} className="text-slate-400 dark:text-zinc-500 hover:text-slate-900 dark:hover:text-white cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
            <p className="text-sm text-slate-600 dark:text-zinc-400 mb-6">Override default plan limits for <strong>{editingTenant.name}</strong>. Leave blank to use their subscribed plan defaults.</p>
            
            <form onSubmit={handleSaveCustomPlan} className="space-y-4">
              <div className="p-3 bg-slate-50 dark:bg-[#09090b] border border-slate-200 dark:border-zinc-800 rounded-xl space-y-2">
                <label className="text-xs font-semibold text-slate-700 dark:text-zinc-300 block">Tenant Logo URL (Displayed on Landing Page Marquee)</label>
                <div className="flex items-center gap-3">
                  {customData.logoUrl ? (
                    <div className="w-10 h-10 rounded-lg border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-1 flex items-center justify-center shrink-0 overflow-hidden">
                    ...
                    <div className="w-10 h-10 rounded-lg border border-dashed border-slate-300 dark:border-zinc-800 bg-slate-100 dark:bg-zinc-900 flex items-center justify-center text-slate-400 dark:text-zinc-500 text-[9px] font-bold shrink-0">
                      No Logo
                    </div>
                  )}
                  <input
                    ...
                    className="flex-1 bg-white dark:bg-[#121214] border border-slate-300 dark:border-zinc-800 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-zinc-500 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:border-emerald-500"
              ...
              <div className="space-y-1">
                <label className="text-xs font-medium text-slate-600 dark:text-zinc-400">Custom Plan Name (optional)</label>
                <input
                  type="text"
                  value={customData.customPlanName}
                  onChange={e => setCustomData({...customData, customPlanName: e.target.value})}
                  className="w-full bg-slate-50 dark:bg-[#09090b] border border-slate-300 dark:border-zinc-800 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-zinc-500 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-emerald-500"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-medium text-slate-600 dark:text-zinc-400">Monthly Price ($)</label>
                  <input
                    type="number"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-slate-600 dark:text-zinc-400">Billing Start Date</label>
                  <input
                    type="date"
                    value={customData.billingCycleStart}
                    onChange={e => setCustomData({...customData, billingCycleStart: e.target.value})}
                    className="w-full bg-slate-50 dark:bg-[#09090b] border border-slate-300 dark:border-zinc-800 text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500 rounded-xl px-3 py-2 text-sm"
                  />
                </div>
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-medium text-slate-600 dark:text-zinc-400">Message Quota /mo</label>
                    {modalUsage && <span className="text-[10px] text-emerald-700 dark:text-emerald-400">Used: {modalUsage.messagesUsed}</span>}
                  </div>
                  <input
                    type="number"
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-medium text-slate-600 dark:text-zinc-400">AI Quota /mo</label>
                    {modalUsage && <span className="text-[10px] text-emerald-700 dark:text-emerald-400">Used: {modalUsage.aiUsed}</span>}
                  </div>
                  <input
                    type="number"
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-medium text-slate-600 dark:text-zinc-400">Team Members</label>
                    {modalUsage && <span className="text-[10px] text-emerald-700 dark:text-emerald-400">Active: {modalUsage.seatsUsed}</span>}
                  </div>
                  <input
                    type="number"
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-medium text-slate-600 dark:text-zinc-400">WhatsApp Limit</label>
                    {modalUsage && <span className="text-[10px] text-emerald-700 dark:text-emerald-400">Active: {modalUsage.currentWhatsapp}</span>}
                  </div>
                  <input
                    type="number"
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-medium text-slate-600 dark:text-zinc-400">Messenger Limit</label>
                    {modalUsage && <span className="text-[10px] text-emerald-700 dark:text-emerald-400">Active: {modalUsage.currentMessenger}</span>}
                  </div>
                  <input
                    type="number"
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-medium text-slate-600 dark:text-zinc-400">Instagram Limit</label>
                    {modalUsage && <span className="text-[10px] text-emerald-700 dark:text-emerald-400">Active: {modalUsage.currentInstagram}</span>}
                  </div>
                  <input
                    type="number"
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-medium text-slate-600 dark:text-zinc-400">Website Widget Limit</label>
                    {modalUsage && <span className="text-[10px] text-emerald-700 dark:text-emerald-400">Active: {modalUsage.currentWebsiteWidget || 0}</span>}
                  </div>
                  <input
                    type="number"
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-medium text-slate-600 dark:text-zinc-400">Products Limit</label>
                    {modalUsage && <span className="text-[10px] text-emerald-700 dark:text-emerald-400 font-mono">Count: {modalUsage.productsCount}</span>}
                  </div>
                  <input
                    type="number"
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-medium text-slate-600 dark:text-zinc-400">Contacts Limit</label>
                    {modalUsage && <span className="text-[10px] text-emerald-700 dark:text-emerald-400 font-mono">Saved: {modalUsage.contactsCount}</span>}
                  </div>
                  <input
                    type="number"
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-medium text-slate-600 dark:text-zinc-400">Storage Limit (MB)</label>
                    {modalUsage && <span className="text-[10px] text-emerald-700 dark:text-emerald-400">Used: {modalUsage.storageUsedMb} MB</span>}
                  </div>
                  <input
                    type="number"
              ...
              <div className="mt-4 border-t border-slate-200 dark:border-zinc-800 pt-4">
                <div className="flex items-center justify-between mb-3">
                  <label className="text-xs font-medium text-slate-600 dark:text-zinc-400">Override System Features (Access Control)</label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input 
                      type="checkbox" 
                      checked={customData.hasFeaturesOverride}
                      onChange={(e) => setCustomData({ ...customData, hasFeaturesOverride: e.target.checked })}
                      className="w-3.5 h-3.5 rounded border-slate-300 dark:border-zinc-700 text-emerald-600 dark:text-emerald-500 focus:ring-emerald-500 accent-emerald-600 cursor-pointer" 
                    />
                    <span className="text-[11px] font-medium text-emerald-700 dark:text-emerald-400">Enable Feature Override</span>
                  </label>
                </div>
                
                {customData.hasFeaturesOverride && (
                  <div className="grid grid-cols-2 gap-3 mt-3">
                    <label className="flex items-center gap-2 cursor-pointer bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-2.5 rounded-xl hover:border-emerald-500/50 transition-colors">
                      <input 
                        type="checkbox" 
                        checked={customData.customAllowByok}
                        onChange={(e) => setCustomData({ ...customData, customAllowByok: e.target.checked })}
                        className="w-4 h-4 rounded border-slate-300 dark:border-zinc-700 text-emerald-600 dark:text-emerald-500 focus:ring-emerald-500 accent-emerald-600 cursor-pointer" 
                      />
                      <span className="text-[11px] font-medium text-slate-800 dark:text-zinc-200">Bring Your Own Key (BYOK)</span>
                    </label>
                    {[
                      ...
                      <label key={feature.id} className="flex items-center gap-2 cursor-pointer bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-2.5 rounded-xl hover:border-emerald-500/50 transition-colors">
                        <input 
                          ...
                          className="w-4 h-4 rounded border-slate-300 dark:border-zinc-700 text-emerald-600 dark:text-emerald-500 focus:ring-emerald-500 accent-emerald-600 cursor-pointer" 
                        />
                        <span className="text-[11px] font-medium text-slate-800 dark:text-zinc-200 leading-tight">{feature.label}</span>
                      </label>
                    ))}
                  </div>
                )}
              ...
              <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 pt-4 border-t border-slate-200 dark:border-zinc-800">
                <button
                  type="button"
                  onClick={handleResetCustomPlan}
                  disabled={saving}
                  className="px-3 py-2 rounded-xl text-xs font-medium text-amber-700 dark:text-amber-400 border border-amber-500/30 hover:bg-amber-500/10 transition-colors disabled:opacity-50 text-center cursor-pointer"
                >
                  Reset Defaults
                </button>
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 sm:flex-initial px-4 py-2 rounded-xl text-sm font-medium text-slate-700 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors text-center cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={saving}
                    className="flex-1 sm:flex-initial px-4 py-2 rounded-xl text-sm font-medium bg-emerald-600 text-white hover:bg-emerald-500 transition-colors disabled:opacity-50 text-center cursor-pointer shadow-md"
                  >
