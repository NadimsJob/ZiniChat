...
| **2026-07-24** | **`epay.corp.com.bd` Redesign: Personal/Merchant Manual Instructions & Fraction Elimination** | `pay-mfs/page.tsx`, `payments.service.ts`, `mfs-payments.service.ts` | Completely redesigned the MFS payment modal to match the `epay.corp.com.bd` design sample (account summary, Send Money/Merchant instructions, order ID reference, auto-verifying banner, and full TrxID / last 4 digits verification input). Eliminated auto-generated dynamic QR code generation and removed fraction offset calculations. |
| **2026-07-24** | **Root Cause Fix: CurrencyProvider `formatBDT` Double-Conversion** | `CurrencyProvider.tsx`, `pay-mfs/page.tsx`, `billing-history/page.tsx`, `subscription/page.tsx` | Identified and resolved the exact root cause of ৳62,377.50 display error: `CurrencyProvider.tsx`'s `formatBDT` function treated all incoming amounts as USD and multiplied them by exchange rate (`rate = 123.76`), converting 504.01 BDT into 62,377.50 BDT. Created `formatBdtDirect` helper for direct BDT formatting without exchange rate multiplication. |
| **2026-07-24** | **Payment Price Calculation Fix, Uploaded QR Priority & Modal Amount Badge** | `payments.service.ts`, `mfs-payments.service.ts`, `pay-mfs/page.tsx` | Fixed package price calculation to strictly use `plan.priceMonthlyBdt` (e.g. 499 BDT) instead of being overridden by `tenant.customPriceUsd` when purchasing standard packages. Prioritized uploaded QR images (`qrCodeUrl`) over dynamic QR generation, and added total amount display inside the payment popup modal. |
| **2026-07-24** | **Dedicated Gateway Checkout Portal, Tenant Billing History & Auto-Cancellation** | `pay-mfs/page.tsx`, `billing-history/page.tsx`, `ClientLayout.tsx`, `mfs-payments.service.ts`, `payments.service.ts` | Built dedicated new-tab gateway portal matching Epay/Paystation UI with dynamic category tabs (Bangla QR default & highlighted), active Superadmin account filtering, real-time timer, and auto-redirect. Created Tenant Billing History page with retry actions, and backend auto-cancellation of duplicate pending/failed invoices upon payment verification. |
| **2026-07-24** | **Android APK Debug Compilation & Webhook JSON Fixes** | `build-android-apk.yml`, `gradle.properties`, `AGENTS.md` | Fixed GitHub Actions APK build failure by adding `gradle.properties` (`android.useAndroidX=true`) and changing workflow to build a **Debug APK** (`assembleDebug`) to bypass Android's "App not installed" restriction for unsigned Release APKs. Deployed backend to live server after diagnosing that third-party SMS Gateway apps fail to parse double-escaped Regex JSON payloads (`\\s`), throwing `400 Bad Request`. Transitioned fully to the zero-config custom ZiniChat APK. |
| **2026-07-24** | **Zero-Config ZiniChat Android Gateway App Upgrade** | `android-sms-gateway/MainActivity.kt`, `SmsReceiver.kt`, `build.gradle`, `.github/workflows/build-android-apk.yml`, `settings/mfs/page.tsx` | Upgraded ZiniChat custom Android app with built-in ON/OFF toggle switches for bKash (& Bangla QR), Nagad, Rocket, and BD Banks (City, BRAC, EBL, DBBL, etc.). Removed all manual JSON/Regex requirements. Added Gradle build structure and GitHub Actions workflow (`build-android-apk.yml`) for automated APK compilation and deployment. |
| **2026-07-24** | **ZiniChat SMS Gateway APK Hosting & Privacy Preservation** | `frontend/public/downloads/zinichat-sms-gateway.apk`, `settings/mfs/page.tsx` | Hosted the official privacy-preserving SMS Gateway APK directly on ZiniChat domain (`/downloads/zinichat-sms-gateway.apk`). Reverted from capcom6 to the sender-filtering APK to prevent forwarding personal SMS to the server while enabling custom JSON template parsing. |
| **2026-07-23** | **Support AI Provider Fix & Role Isolation** | `support-chat.service.ts`, `middleware.ts`, `dashboard/page.tsx` | Fixed AI Chat Support failure by adding `baseURL` resolution (`https://generativelanguage.googleapis.com/v1beta/openai/`) for Gemini provider and model fallbacks. Fixed cookie/token bleed by isolating `superadmin` role sessions from `/dashboard` in `middleware.ts`, and removed redundant `Current Plan` header elements. |
| **2026-07-23** | **Webhook Verify Token Fallback Fix** | `whatsapp.controller.ts`, `messenger.controller.ts` | Added default fallback `zinichat_secret_webhook_token_2026` to WhatsApp and Messenger controllers' `@Get()` webhook verification methods, enabling 200 OK challenge responses during Meta Developer Portal webhook subscription. All 89 unit tests passed. |
| **2026-07-23** | **USD Fraction Preservation & Tenant Subscription Sync** | `CurrencyProvider.tsx`, `subscription/page.tsx`, `PricingSection.tsx` | Preserved up to 2 decimal places for USD prices to prevent revenue loss while keeping BDT rounded to integers. Synchronized the BDT ↔ USD currency toggle, dynamic discount badges, and highlighted yearly totals to the Tenant Dashboard Subscription Settings page. Next.js build verified (57/57 routes passed). |
| **2026-07-23** | **Pricing Currency Conversion & Dynamic Discount Overhaul** | `PricingSection.tsx`, `pricing/page.tsx`, `CurrencyProvider.tsx` | Fixed USD currency conversion on landing page and pricing page. Enforced English numerals when USD currency is selected regardless of active UI language. Made the "Save up to X%" toggle badge dynamically calculate max package discounts, rounded yearly monthly equivalents to clean integers (no fractions), and highlighted the yearly total billing banner. All 89 unit tests passed. |
| **2026-07-23** | **Official Contact & Admin Email Routing** | `landing-page.service.ts`, `contact/page.tsx`, `smtp.service.ts` | Updated platform official contact info to Phone: `01533894967` and Address: `#386, Uttar Badda, Dhaka-1212, Bangladesh`. Configured `support@zinichat.com` as the default recipient for all Superadmin system alert emails (e.g. pending payments, new inquiries, new support tickets). |
| **2026-07-23** | **Official & Support Email Standardization** | `landing-page.service.ts`, `(marketing)/contact/page.tsx` | Standardized platform email addresses across marketing pages and backend defaults: `info@zinichat.com` for official business/landing page contact info, and `support@zinichat.com` for technical support. Automatically migrates old default config emails. |
| **2026-07-23** | **Meta Developer App Config & Multi-Channel Guide** | `facebook-auth/page.tsx`, `auth.service.spec.ts` | Overhauled Superadmin Facebook Auth setup UI to display complete multi-channel OAuth Redirect URIs (WhatsApp & Instagram/Messenger) and Webhook endpoints (`/webhooks/whatsapp` & `/webhooks/messenger`). Added a step-by-step setup guide and unit tests for Facebook settings (`auth.service.spec.ts`). All 89 unit tests pass successfully. |
| **2026-07-23** | **MFS Gateway Android App Link & Unit Testing** | `settings/mfs/page.tsx` | Updated SMS Gateway download link to point to active release fork (v1.67.0) allowing direct APK downloads without manual Android Studio compilation. Restored JSON template configuration guide for third-party APK support. Ran full test suite (16 suites / 89 tests passing). |
| **2026-07-23** | **SMTP Template Standardization & Unit Testing** | `smtp.service.ts`, `smtp.service.spec.ts` | Refactored `TEMPLATES` constant to securely include standard welcome templates. Completely rewrote the `smtp.service.spec.ts` suite to cover all 15 dynamic email triggers (e.g. welcome, payment, addon, password reset, tickets, inquiries) verifying proper placeholder interpolation. All 18 tests passed successfully. |
| **2026-07-23** | **Tenant Addon Provisioning & Sync** | `frontend/src/app/(tenant)/dashboard/billing/pay-mfs/page.tsx`, `frontend/src/app/(tenant)/dashboard/settings/subscription/page.tsx`, `billing.service.spec.ts`, `smtp.service.spec.ts` | Implemented end-to-end MFS Addon checkout flow for tenants. Hides coupon inputs during addon purchases and synchronizes quota upgrades by accumulating the addon value on top of existing base plan or custom plan overrides. Fixed mocking logic in backend unit tests (Billing & SMTP modules) so all 73 backend tests pass successfully. |
| **2026-07-23** | **Multi-Provider AI Configurations** | `ai.service.ts`, `ai.controller.ts`, `settings/ai/page.tsx`, `ai.service.spec.ts` | Added native support for Gemini and Anthropic Claude APIs alongside OpenAI. Configured `fetchModels` and `generateCompletion` to dynamically handle provider-specific headers and payload structures based on a new UI provider dropdown. Created unit tests. |
| **2026-07-23** | **Dynamic Tenant Dashboard Overhaul** | `dashboard/page.tsx`, `tenant-stats.service.ts` | Completely redesigned the tenant dashboard into a modular, detailed reporting screen. Added strict Setup-First lock screen to hide stats until setup is complete. Built dynamic blocks for Core Quotas, E-Commerce, and CRM Leads based on active plan features, plus a real-time Recent Activity feed. Fixed TS2339 JSON length errors during build. |
