...
| **2026-07-23** | **Dashboard Onboarding & UI Bug Fixes** | `dashboard/page.tsx`, `SetupJourneyWidget.tsx`, `ClientLayout.tsx`, `subscription/page.tsx` | Fixed subscription pricing number formatting for BDT. Fixed the dashboard layout to unconditionally show the Support Widget for all plans. Updated Setup Journey checklist to always show items for default plans. Added Upgrade Plan button and working Support Widget trigger to the Free Setup banner. |
| **2026-07-23** | **Custom Pricing & Plan Sync** | `billing.service.ts`, `payments.service.ts`, `settings/subscription/page.tsx` | Synchronized superadmin tenant custom plan overrides (`customPriceUsd`, `customPlanName`) with the tenant dashboard and the MFS checkout payment gateway. Custom USD prices are now dynamically converted to BDT at checkout, replacing the base plan pricing during manual payment submissions. |
| **2026-07-23** | **Checkout Coupons Sync & Superadmin Billing fixes** | `pay-mfs/page.tsx`, `superadmin/billing/page.tsx`, `superadmin/payments/page.tsx` | Added coupon input/apply/remove functionality to the MFS checkout page, dynamically updating BDT pricing calculations and generating discounted Bangla QR codes. Synced superadmin payments and billing pages with active environment configs, BDT currency, and correct database fields. |
